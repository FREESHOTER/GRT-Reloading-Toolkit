...
Community plugin for [Gordon's Reloading Tool](https://grtools.de). One plugin, twenty-two tools.
...
| 📈 **Ladder / OCW Analyzer** | Charge ladder (chrono velocities + target groups) → velocity flat-spot (Satterlee) + vertical-POI node (OCW/Audette) + weighted best node → an *OCW Analysis* note **and a chart** in the load. Groups come from OnTarget `.csv` **or straight from GRT's own "Shot group" tabs** in the load ("Groups from GRT load", with reference/shooting-distance units and an optional drop-flyers toggle). Missing velocities are filled in from the load's own chrono Measurement. |
...
| 📒 **Inventory** | Component inventory (powder / primer / brass / bullet / barrel) with lot tracking, brand pick-lists, powder counted in **g, gr, lb or kg**, twist and length on barrels, and cost per round; a **Firearms** tab (round-count per barrel + MV-drift chart); a **Brass Life** tab (avg uses/case, anneal-due and retirement-due status per lot, from Journal-logged round counts); a **Pressure Signs** tab (fired-case head diameter per charge, flagging a charge-to-charge expansion-rate jump against that same ladder's own trend). SQLite in `%AppData%\GRTPlugins\`, shared with Load Journal and Find best Ba. |
...
| 🌡🎯 **SD Root-Cause** | For one chronograph string: leave-one-out ranking of which shot(s) drag the SD down, a Single/Partial/Systematic outlier pattern, and every triggered cause (cold bore, progressive drift, temperature↔velocity, temperature↔group dispersion when a matching GRT Shot-group tab exists) — no setup needed first. |
| 🎯📐 **Group Analysis** | The 2D sibling of SD Root-Cause: for one shot group (OnTarget `.csv`, GRT's own Shot-group tabs, or manual entry), CEP50/extreme-spread/mean-radius metrics, Mahalanobis-based outlier flags, a Score (0-10) and a sample-size-driven Confidence kept as two separate numbers, a problem list with the evidence behind each item, group-combining by recentring, and an optional velocity↔dispersion correlation against a matching Athlon folder. |
| 🎯🖨 **Print Ladder/OCW Target** | A true-scale printable OCW or Ladder/Audette target, prepared before the range — mm **and** inch scales, 1-5 shots per aim point, optional charge labels, paginated for long ladders. Print at 100% on A3, or **Save PDF…** through Windows' "Microsoft Print to PDF" for a guaranteed-true-A3 file regardless of which printer is selected. No GRT connection needed. |
| 📚 **Load Book Export** | Walks a folder of `.grtload` files (recursively, one row per real recipe even alongside this toolkit's own write-back siblings) and renders one HTML document grouped by caliber: powder, charge, bullet, COAL, `Ba`/`a0`, and whatever the Journal — or the file's own embedded shot data — measured. No GRT connection needed. |

Plus **📄 Install GRT report templates** (launcher button / Plugins menu): writes sixteen DokuWiki
...
Every note-writing tool has its own dedicated page now, except Seating Force Estimate (QC), the
Guided New Load wizard, Print Ladder/OCW Target and Load Book Export — none of the four write
anything back to an open GRT load (the last two need no GRT connection at all).
